...
TEST_CASE("Static members assignment", "[mat_construct]") {
...
TEST_CASE("at() usable to access reference", "[mat_access]") {
  using Mat = vecpp::Mat<int, 2, 2>;
...
TEST_CASE("use data() to access underlying data", "[mat_access]") {
  using Mat = vecpp::Mat<int, 2, 2>;
  {
    const Mat a = {1, 2, 3, 4};
